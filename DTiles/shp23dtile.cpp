...
public:
	bbox _box;
	// 1 公里 ~ 0.01
	double metric = 0.01;//0.01
	node* subnode[4];
	std::vector<int> geo_items;//只有叶子节点有
	std::vector<int> children_items;//每一级别都有
public:
	int _x = 0;
	int _y = 0;
	int _z = 0;
...
public:
	...
	void add(int id, bbox& box) {
		...
		children_items.push_back(id);//都放进去
	...
	std::vector<int>& get_children_items()
	{
	...
	//获取所有层级
	int get_all_Level(std::vector<void*> items_array)
	{
		int temp = 0;
	...
	//获取某层级节点
	void get_SpecifLevel_Nodes(int level, std::vector<void*>& items_array_level)
	{

		if (this->_z == level) {
			items_array_level.push_back(this);
		}
		if (subnode[0] != 0) {
			for (int i = 0; i < 4; i++) {
				subnode[i]->get_SpecifLevel_Nodes(level, items_array_level);
			}
		}
	...
	const char* dest, const char* height)
{
	...
	for (auto item : items_array) {
		node* _node = (node*)item;
		char b3dm_file[512];
		std::sprintf(b3dm_file, "%s\\tile\\%d\\%d", dest, _node->_z, _node->_x);
		mkdirs(b3dm_file);
		// fix the box 
		{
		...
		std::sprintf(b3dm_file, "%s\\tile\\%d\\%d\\%d.b3dm", dest, _node->_z, _node->_x, _node->_y);
		...
		std::sprintf(b3dm_name, "./tile/%d/%d/%d.b3dm", _node->_z, _node->_x, _node->_y);
		std::sprintf(tile_json_path, "%s\\tile\\%d\\%d\\%d.json", dest, _node->_z, _node->_x, _node->_y);
		double box_width = (_node->_box.maxx - _node->_box.minx);
		double box_height = (_node->_box.maxy - _node->_box.miny);
		const double pi = std::acos(-1);
		double radian_x = degree2rad(center_x);//当前盒子中心点x 的 弧度
		double radian_y = degree2rad(center_y);//当前盒子中心点y 的 宽度
...
extern "C" bool shp23dtileBylevel()
{
	//层级数量、每层级切面跨度（x轴）、GeometricError
	return false;
}

extern "C" bool shp23dtile(
	const char* filename, int layer_id,
	const char* dest, const char* height,int& flag)
{
	flag = 0;
	float temp_flag = 0;
	if (!filename || layer_id < 0 || layer_id > 10000 || !dest) {
	...
	// iter all node and convert to obj 
	std::vector<void*> items_array;
	root.get_all(items_array);//得到所有的
	//
	int field_index = -1;
	if (!height_field.empty()) {
		field_index = poLayer->GetLayerDefn()->GetFieldIndex(height_field.c_str());
	...
	for (auto item : items_array) {
		...
		temp_flag = (temp / (items_array.size())) * 90;
...
tinygltf::Material make_color_material(double r, double g, double b) {
	...
	char buf[512];
	std::sprintf(buf, "default_%.1f_%.1f_%.1f", r, g, b);
	material.name = buf;
