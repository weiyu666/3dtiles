/*
* 该库实现了一种改进的ear切片算法，通过z阶曲线散列进行优化，并以一种不保证三角剖分正确性的方式扩展到处理孔、
* 扭曲多边形、退化和自相交，但尝试始终为实际数据（如地理形状）生成可接受的结果。
*它基于FIST的思想：Martin Held提出的多边形的快速工业强度三角剖分和David Eberly提出的“削耳”三角剖分。
*/
