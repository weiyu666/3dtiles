...
/*
* 该模块主要是extern.h的函数实现方式，按照实现命名文件3d tiles的设置
*/
...
extern "C" bool mkdirs(const char* path)
{
    ...
    */
    ...
    for (int i = 0; (NULL != strchr(path + iCreatedLength, '\\')); i++)
    {
        ...
        strncpy(szPathTemp, path, iCreatedLength);
        mkdir(szPathTemp);
    }
...
{
    std::cerr << "Catch Exception : " << msg << std::endl;
...
/**
根据经纬度，生成tileset 中心点X弧度、中心点Y弧度、宽度（米）、高度（米）、最小高度、最大高度、GE
*/
        bool write_tileset(
